...
class PayrollReportView(BackendMixin, ListView):
    ...
    def get_queryset(self):
        ...
        sessions = Session.objects.filter(date__gte=start, date__lte=end)\
                                  .values_list('course', 'instructor', 'export_date')
        exported_count = {}
        not_exported_count = {}
        for (course_id, instructor_id, export_date) in sessions:
            if export_date is not None:
                ...
                not_exported_count[(course_id, instructor_id)] = not_exported_count.get(
                    (course_id, instructor_id), 0) + 1
