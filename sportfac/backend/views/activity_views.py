...
class ActivityAbsenceView(BackendMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        qs = Absence.objects.select_related('child', 'session', 'session__course',).filter(session__activity=self.object)
        ...
        if settings.KEPCHUP_REGISTRATION_LEVELS:
            ...
            kwargs['child_levels'] = dict(
                [(lvl.child, lvl) for lvl in ChildActivityLevel.objects.filter(activity=self.object)
            ...
            try:
                ...
                all_extras = dict(
                    [(extra.registration.child, extra.value) for extra in
                     ExtraInfo.objects.filter(registration__course__activity=self.object, key__in=questions)
