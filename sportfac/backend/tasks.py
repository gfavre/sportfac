...
@respects_language
def create_tenant(
    start, end, copy_activities_from_id=None, copy_children_from_id=None, user_id=None
):
    ...
    with transaction.atomic():
        ...
        Domain.objects.get_or_create(
            ...
            domain="%s-%s" % (start.strftime("%Y%m%d"), end.strftime("%Y%m%d")),
        )
    try:
    ...
    if copy_activities_from_id:
        ...
        try:
            ...
            logger.warning(
                "Year tenant {} (source) does not exist".format(copy_activities_from_id)
            )
