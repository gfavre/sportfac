...
from ...views.course_views import (
    CourseCreateView, CourseAbsenceView, CoursesAbsenceView,
    CourseDeleteView, CourseDetailView, CourseListView, CourseUpdateView,
)
...
class CoursesAbsenceViewTests(TenantTestCase):
    def setUp(self):
        ...
        self.login_url = reverse('login')
        self.url = reverse('backend:courses-absence') + '?c={}&c={}'.format(self.course1.pk, self.course2.pk)
        self.user = FamilyUserFactory(is_manager=True)
