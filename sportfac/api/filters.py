...
def cleanup_value(model_class, accessor, values):
    ...
    if hasattr(model_field, "choices") and model_field.choices:
        # SearchPanes sends back whatever it was given as each option's "value" in
        # get_search_panes() - the actual choice key (e.g. "canceled"), confirmed via a
        # real browser request against dt-1.10.24/sp-1.2.1 - not its label ("Annulée"),
        # despite what an older SearchPanes release apparently did (hence the label
        # reverse-lookup kept below as a fallback, in case some client ever does send a
        # label instead - it's the same _display_map either way, just inverted here).
        # noinspection PyProtectedMember
        display_map = model_field.choices._display_map
        label_to_key = {v: k for k, v in display_map.items()}
        return f"{accessor}__in", [value if value in display_map else label_to_key.get(value) for value in values]
