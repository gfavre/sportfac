...
def _create_domain():
    ...
    for domain_info in env.webfaction.list_domains():
        subdomains = domain_info.get('subdomains') + ['.']
        if domain_info.get('domain') == env.domain and \
           set(subdomains) & set(env.subdomains) == set(env.subdomains):
...
def _create_website():
    ...
    website_fct(env.website_name, ip, env.https,
                env.allowed_hosts,
                (env.django_app_name, '/hiver'),
                (env.static_app_name, '/hiver/static'),
                (env.media_app_name, '/hiver/media'))
