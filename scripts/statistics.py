...
for reg in Registration.objects.select_related('course', 'course__activity').order_by('created'):
    print('\t'.join((str(reg.created.astimezone(to_zone)), reg.course.activity.name, str(reg.course.number))))
...
for reg in Registration.objects.select_related('course', 'course__activity').order_by('created'):
    ...
    if course_nb[reg.course] == reg.course.max_participants:
        print('\t'.join((str(reg.created.astimezone(to_zone)), reg.course.activity.name, str(reg.course.number), 'Cours complet')))
        all_full = True
