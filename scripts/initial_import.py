...
with open('/Users/grfavre/Desktop/cours.csv') as csvfile:
    ...
    reader.next()
    for (a_name, junk, junk, price, resp, nb, day, junk, start_date, end_date, start_time, end_time, place, nbmin, nbmax, year, trim) in reader:
        if a_name and a_name not in activities:
            activity, created = Activity.objects.get_or_create(name=a_name.decode('latin-1'), slug=slugify(a_name.decode('latin-1')))
            activity.save()
            activities[a_name] = activity
        if resp and resp not in responsibles:
            names = resp.decode('latin-1').split(' ')
            resp_obj, created = Responsible.objects.get_or_create(last=len(names) > 1 and names[1] or names[0], 
        ...
        course = Course(activity=activities[a_name], responsible=responsibles[resp],
                        price = int(price), number_of_sessions=int(nb),day = days[day],
                        ...
                        schoolyear_min = ymin, schoolyear_max = ymax)
        course.save()
