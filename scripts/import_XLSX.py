...
for (registration_id, id_lagapeo, bib_number, transport_info, last, first) in dataset:
    try:
        ...
        try:
            ...
            c2.delete()
            child.save()
        print(("Child {} updated".format(child.full_name)))
...
from import_export.formats.base_formats import XLSX
from registrations.models import Child, ChildActivityLevel, Registration
...
for (
    ...
    try:
        ...
        for extra in registration.extra_infos.all():
            if extra.key.question_label == "Niveau de ski/snowboard":
                if extra.value != level:
                    print(("{} ≠ {}".format(extra.value, level)))
                    extra.value = level or ""
                    extra.save()
            elif extra.key.question_label == "Arrêt de Bus":
                if extra.value != train:
                    print(("{} ≠ {}".format(extra.value, train)))
                    extra.value = train or ""
                    extra.save()
            elif extra.key.question_label == "Votre enfant aura-t-il un abonnement de ski saison?":
                if extra.value != abo_annonce:
                    print(("{} ≠ {}".format(extra.value, abo_annonce)))
            elif extra.key.question_label == "Arrêt de train":
                if extra.value != train:
                    print(("{} ≠ {}".format(extra.value, train)))
                    extra.value = train or ""
                    extra.save()
            elif extra.key.question_label == "Snowboard ou ski?":
                pass
...
from import_export.formats.base_formats import XLSX
from registrations.models import Child, ChildActivityLevel, Registration, Transport
